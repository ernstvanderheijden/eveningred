...
crudname = 'crudconditiontype'


class Conditiontypelist(GlobalList):
    def __init__(self, request, viewtype, pk=None):
        super().__init__(request)
        self.contenttitle = 'Betalingstermijnen'
        ...
        if request.user.has_perm('master.add_conditiontype'):
            self.tools.update({
                'create': {
                    "title": "fas fa-plus",
                    "tooltype": "create",
                    "class": "btn btn-primary",
                    "url": "/core/create/?level=" + str(int(self.level) + 1) + "&package=" + self.package + "&crud=crudconditiontype&fk=&nameform=" + nameform + "&successurl=" + encode_string(self.successurl_decoded)
                },
            })
        ...
        self.records = Conditiontype.objects.filter().values(*self.columnfields.keys(), "id", "status")
