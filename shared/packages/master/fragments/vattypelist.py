...
crudname = 'crudvattype'


class Vattypelist(GlobalList):
    def __init__(self, request, viewtype, pk=None):
        super().__init__(request)
        self.contenttitle = 'BTW types'
        ...
        if request.user.has_perm('master.add_vattype'):
            self.tools.update({
                'create': {
                    "title": "fas fa-plus",
                    "tooltype": "create",
                    "class": "btn btn-primary",
                    "url": "/core/create/?level=" + str(int(self.level) + 1) + "&package=" + self.package + "&crud=crudvattype&fk=&nameform=" + nameform + "&successurl=" + encode_string(self.successurl_decoded)
                },
            })
        ...
        self.records = Vattype.objects.filter().values(*self.columnfields.keys(), "id", "status")
